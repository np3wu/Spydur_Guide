...
## Normal termination in Gaussian
...
or
```
Erroneous write. write - I instead of 3648000.
fd = 4
writwa writwa: No space left on device
```
or
```
Erroneous write during file extend. write - I instead of 8192
```
Probably out of disk space. Write error in NtrExt I

#### Explanation of error

Typically, this occurs when you have run out of disk space. This could occur if you have exceeded your quota, ifa disk is physically full, or (more rarely) a network drive is unavailable because of a communication time out.
...
Error termination request processed by link 9999. Error termination via Lnk1e in /disc30/g98/I9999.exe.
...
## angle Alpha is outside the valid range of O to 180

#### Description of error

At the end of your output, you get a line such as
```
Error termination via Lnk1e in /disc30/g98/17 I 6.exe.
```

The lines above will be a z-matrix, above which will contain lines such as
```
Error on Z-matrix card number 9
angle Alpha is outside the valid range of0 to 180. 
Conversion from Z-matrix to cartesian coordinates failed:
--------------------------------------------------------------------------------
Z-MATRIX (ANGSTROMS AND DEGREE)
CD Cent Atom NI Length/X N2 Alpha/Y NJ Beta/Z J
--------------------------------------------------------------------------------
9 9 11 8 0.962154( 8) I -1.879( 16) 2 0.000(23) 0
```
#### Explanation of error
...
At the end of your output, you get a line such as
```
End of file reading basis center. Error termination via Lnk1e in /disc30/g98/130 I .exe.
Job cpu time: 0 days 0 hours 0 minutes 1.9 seconds.
File lengths (MBytes): R WF= 11 Int= 0 D2E= 0 Chk= 10 SCr=  1
```
#### Explanation of error
...
At the end or your output, you get a line such as
```
Error termination in NtrErr: NtrErr Called from FilelO.
```

Above this, you have something like Operation on file out of range.
```
Fi le10: IOpcr= 2 IFilNo( I )=-19999 Len= I 829888 IPos= -900525056 Q= 43 52094416

dumping /fiocom/, unit= 1 NFilcs = 109 SizExt = 524288 WlnBlk =1 024 defal = T LstWrd = 7437256704 FType=2 FMxFil=l0000
```
...
At the end of your output, you get a line such as
```
Symbolic Z-matrix: End of file in GetChg.
Error termination via Lnk1e in /disc30/g98/l 101.exe. Job cpu time: 0 days 0 hours 0 minutes 0.5 seconds.
File lengths (MBytes): R WF= 6 Int= 0 D2E= 0 Chk= 11 Sc,-
```
#### Explanation of error
...
At the end of your output, you get a line such as
```
Stoichiometry Cdl-1I4O7(2 ) Framework group C2\[C2(CdO),X(H14O6)\]
Deg. of freedom 30
Full point group C2 NOp 2
Omega: Change in point group r standard orientation.
Error termination via Lnk1e in /disc30/g98/I202.exe. Job CJ u time: 0 days 3 hours 35 minutes 40.8 second .
File lengths (MBytes): R WF= 58 Int= 0 D2E= 0 Chk= 19 Ser=
```
#### Explanation of error

During the optimization proc ss, either the standard orientation or the point group of the molecule has changed. If the former, a visualization program will show this as a sudden flipping of the structure, typically by 180 degrees. This error is a lot less common since Gaussian 03 was released.

#### Fixing the error

- This can indicate that your z-matrix is not correctly specified, if you go from a point group (e.g C2v) to a subgroup of the point group (C2 or Cs or CI).
    ...
    - Rarely: If the point group here is incorrect (or higher symmetry), then your z-matrix should be reformulated with more symmetry.
...
Error termination via Lnk1e in /disc30/g98/I30 I .exe. Job cpu time: 0 days 0 hours 0 minute 1.6 second .
File lengths (MBytes): R WF= 6 Int= 0 D2E= 0 Chk= 12 Ser= 1
```
#### Explanation of error
...
SCF- Done: E(RIIF)= -2131.95693715 A.U. after 257 cycles 
...
Job cpu time: 0 days 0 hours 5 minutes 0.5 seconds. Filelengths (MBytes): RWF= 15 Int= 0 D2E= 0 Chk= 8 Scr= 1
```
or
```
>>>>>>>>>> Convergence criterion not met. 
...
S**2 = 0.8616
Annihilation or the first spin contaminant: 
S\*\*2 before annihilation 0.8616, after 0.7531
Convergence failure -- run terminated.
Error termination via Lnk1e in /disc30/g98/l502.exe. Job cpu time: 0 days 0 hours 3 minutes 56.2 seconds.
File lengths (MBytes): R WF= 11 Int= 0 D2E= 0 Chk= 8 Ser= 1
```
#### Explanation of error
...
- Try using a better guess (guess=read) by carrying out an SCF using the same starting structure, but at a lower level of theory such as Hf'/STO-JG.
...
Error termination via Lnk1e in /disc30/g98/1202.exe. 
Job cpu time: 0 days O hours O minutes 1.3 seconds.
File lengths (MBytcs): R WF= I I Int= 0 D2E= 0 Chk= I Ser= I
```
#### Explanation of error
...
Unable to project read-in occupied orbitals. 
Error termination via Lnk1e in /disc30/g98/1401.exe. 
Job cpu time: 0 days O hours O minutes 29.5 seconds.
File lengths (MBytes): R WF= 18 Int= 0 D2E= 0 Chk= 17 Ser=
```
#### Explanation of error

You are reading in a molecular-orbital guess from the checkpoint file, but the projection from the old to the new basis set has failed. This can happen if certain pseudopotential basis sets (CEP-121G\*) are used with polarization functions where no polarization functions actually exist. In some ca cs Gaussian uses placeholder polarization functions with zero exponent.
...
Job cpu time: 0 days O hours O minutes 32.7 seconds.
File lengths (MBytes): R WF= 514 Int= 0 D2E= 0 Chk= IO Ser=
```
#### Explanation of error
...
At the end of your output, you get Lines such as
```
Symbol "H3NNN" not found in Z-matrix. Error termination via Lnk1e in /disc30/g98/l 10 I .exe.
Job cpu time: 0 days O hours O minutes 0.5 seconds.
File lengths (MBytes): R WF= 6 Int= 0 D2E= 0 Chk= 14 Sci=
```
#### Explanation of error
...
## Variable X has invalid nu1nber of steps

#### Description of error

At the end of your output, you get lines such as
```
Scan the potential surface. Variable Value No. Steps Step-Size
Variable I has invalid number of steps -1.
Error termination via Lnk1e in /disc30/g98/l 108.exe.
.lob cpu time: 0 days O hours O minutes 0.7 seconds.
File lengths (M13ytes): R WF= 11 Int= 0 D2E= 0 Chk= 13 Ser=
```
#### Explanation of error

This is an input error. You are attempting to do a generate rigid potential energy scan. Mo t likely, you have two blank lines instead or one between the z-matrix and the variables.
...
Al the end of your output, you get lines such as

Problem with the distance matrix. Error termination via Lnk1e in /disc30/g98/1202.exe. Job cpu time: 0 days 9 hours I I minutes 14.3 seconds.

File lengths (M13ytes): RWF= 634 Int= 0 D2E= 0 Chk= IOSer= I
...
You will need to check variables and the z-matrix of the atoms in question to make sure there are no atoms close together. This can be a result of a missing minus sign in a torsion angle for molecules with planes of symmetry, in which the two atoms related by the plane or symmetry are now coincident, that is, they have distance O between them.
...
At the end of your output, you get lines such as
```
Symbolic Z-matrix: Charge= 0 Multiplicity= I End of file in ZSymb.
Error termination via Lnk1e in /disc30/g98/l 10 I .exe. Job cpu time: 0 days O hours O minutes 0.6 seconds.
File lengths (MBytes): RWF= 6 Int= 0 D2E= 0 Chk= 9 Sc,= I
```
#### Explanation of error
...
Adel a blank line at the end or acid geom=check.
...
Linear search skipped for unknown reason.
Error termination via Lnk1e in /disc30/g98/l 103.exe.
Job cpu time: 0 clays 7 hours 9 minutes 17.0 seconds.
filelengths(Ml3ytes): RWF= 21 Int= 0D2E= 0Chk= 6Scr= I
```
#### Explanation of error

The rational function optimization was not successful during a linear search. Mo t likely the Hessian is no longer valid.
...
Termination in UpdVrl. Error termination via Lnk1e in /clisc30/g98/l 10 I .exe.
Job cpu time: 0 clays 0 hours 0 minutes 0.5 seconds.
File lengths (MBytes): RWF= 11 Int= 0 D2E= 0 Chk= 8 Ser= I
```
#### Explanation of error
...
At the end of your output, you get lines such as
```
Unknown center X Error termination via Lnk1e in /disc30/g98/l 101.exe. Job cpu time: 0 days 0 hours 0 minutes 0.5 seconds.
File lengths (MBytes): R WF= 6 Int= 0 D2E= 0 Chk= 8 Ser=
```
#### Explanation of error
...
Error termination via Lnk1e in /disc30/g98/19999.exe. 
Job cpu time: 0 days I hours 53 minute I 0.4 seconds.
File lengths (MBytes): RWF= 20 Int= 0 D2E= 0 Chk= 11 Ser= I
```
and just before
```
Determination of dummy atom variables in z-matrix conversion failed. Determination of dummy atom variables in z-matrix conversion failed. 
NNew= 6.03366976D+0 I NOld= 5.07835896D+0 I Diff= 9.55D+00
```
#### Explanation of error
...
This is not strictly a Gaussian error. It typically means that the computer has run out of memory, perhaps because you asked for too much memory in the %mcm line.

#### Fixing the error

- Reduce the amount specified with %mem.
- Or, increase the amount or memory requested in the job script with --mem=.
...
Z-Matrix taken from the checkpoint tile:
oxalate_2-\_AqI_C2.chk Charge and multiplicity card seems defective:
Charge is bogus. WANTEDAN INTEGER AS INPUT. POUND A STRING AS INPUT.
CX = 0.7995

?

Error termination via Lnk1e in /disc30/g98/IIOI .exe.
```
#### Explanation of error

This is an input error. If the title line is forgotten when using geom=modify, then it interprets the charge/multiplicity line (in the above example, "-2 I") as the title, and then tries to interpret the variable list as the charge/multiplicity line.
...
File lengths (MBytes): R WF= 6 Int= 0 D2E= 0 Chk= 8 Sci=
```
#### Explanation of error
...
At the end of your output, you get lines such as
```
Standard basis: 3-21G (60, 7F) Inconsistency #2 in MakNEB.
Error termination via Lnk1e in /disc30/g98/1301.exe. Job cpu time: 0 days 3 hours 46 minutes 57.4 seconds.
File lengths (MBytes): RWF= 245 Int= 0 D2E= 0 Chk= I I Ser= I
```
#### Explanation of error

This is an input error. You have had a change in point group, and you specified iop(2/ I 5=4,2/ I 6=2,2/17=7) to try to avoid program crashing.

#### Fixing the error

Be very careful with iop, or remove altogether.

## galloc: could not allocate 1ne111ory
...
This is a memory allocation error due to lack of memory. Gaussian handles memory in such a way that it actually uses about I GB more than %mem.

#### Fixing the error

The value for %mem should be at least I GB less than the value specified in the job submission script. Conversely, the value specified for --mem in your job script should be at least I GB greater than the amount specified in the %mem directive in your Gaussian input file. The exact increment needed seems to depend on the job type and input details; 1GB is a conservative value determined empirically.
...
PGFIO-F-/OPEN/unit==l I/error code returned by host stdio - 2. 
File name==/home/johndoe/scratch/Gau-12345.inp
In source file ml0.f, at Iine number 181
0 0x42bb41 Error: segmentation violation, address not mapped to object
```
#### Explanation of error

The file named in the third line doesn't exist, possibly because the containing directory docs not exist. This happens, for example, if you set GAUSS_SCRDIR to a directory that doesn't exist.
